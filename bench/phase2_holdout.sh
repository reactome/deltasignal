#!/usr/bin/env bash
# Phase 2: held-out test on the curator pathways (excluding the 10 used for
# tuning). Runs one or more (AND_MODE, INHIBITION_MODE) configs and dumps
# a summary so we can compare to the naive Boolean baseline (~70.55% e2e).
#
# Usage:
#   bash bench/phase2_holdout.sh            # runs the two configs we care about
#   bash bench/phase2_holdout.sh AND INHIB  # runs a single config
set -eo pipefail

MAX_EDGES=80000
DOWN=0.5
UP=2.0
REPORT_DIR=/tmp/ds_phase2
mkdir -p "$REPORT_DIR"
...
run_one() {
    ...
    echo
    echo "=== HELD-OUT: AND=${and}  INHIB=${inhib}  thresholds=(${DOWN}, ${UP}) ==="
...
if [ $# -eq 2 ]; then
    ...
    # Default: compare current winner (signed,spec) vs (signed,inversion).
    for inhib in spec inversion; do
        restart_api signed "$inhib"
        run_one signed "$inhib"
    done
fi

echo
echo "==================== HELD-OUT SUMMARY ===================="
column -t -s $'\t' "$SUMMARY"
echo
echo "Naive Boolean baseline (reference): 70.55% e2e on this same held-out set"
